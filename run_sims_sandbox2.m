...
consumerVars_initial = exprnd(.05,numAspects,1);
...
%population_means =  0.5*consumerMeans_initial+0.5*m2';
%recommenderMeans_initial = population_means;
%recommenderMeans_initial = weight_truth*population_means + (1-weight_truth)*consumerMeans_initial;

%recommenderMeans_initial = mean(all_true_partworths);
...
%  recommenderMeans_initial(find(dummy>set_to_0))=true_partworths(find(dummy>set_to_0));
 ...
 recommenderMeans_initial = weight_truth*consumerMeans_initial+(1-weight_truth)*true_partworths;
...
recommender_utils = allProducts_binary*recommenderMeans_initial';
%%
if no_rec ==1;
...
%%
if rec2==0;
if netPayoff_no_rec/max(all_utils)<.8
    all_recs = 1;
    if all_recs ==1;
    ...
    figure; scatter(all_utils,all_netPayoffs)
end
end
...
if rec2 ==1;
    if netPayoff_no_rec/max(all_utils)<.8
        if all_recs==1
            all_pairs = combntns([1:numProducts],2);
            all_netPayoffs = zeros(1,size(all_pairs,1));
            for pair = 1:size(all_pairs,1)
                products = all_pairs(pair,:);
                current_means = consumerMeans_initial;
                searchedAspects=max(allProducts_binary(products(1),:),allProducts_binary(products(2),:));
                current_means(find(searchedAspects))=true_partworths(find(searchedAspects));
                current_ustar = zeros(numAttrs,1); %this is assuming there is an additional attr level with known partworth 0
                for attr = 1:numAttrs
                    current_ustar(attr)=max(0,max(current_means(beginAttr(attr):endAttr(attr))));
                end
                time = 2;
                run_search
                all_netPayoffs(pair)=netPayoff;
                all_searchTimes(pair)=time;
                all_mean_utils(pair)=sum(all_utils(products(1))+all_utils(products(2)))/2;
                if all_netPayoffs(pair)<sum(all_utils(products))/2-time*cost;
                    keyboard
                end
            end
        end
        figure; scatter(all_mean_utils,all_netPayoffs)
    end
end
 for pair = 1:size(all_pairs,1)
     products = all_pairs(pair,:);
     all_mean_recommender_utils(pair)=sum(recommender_utils(products(1))+recommender_utils(products(2)))/2;
 end
